...
promedioMochilaMenorA3 = []
nMasmMenos1 = []
while k < (len(tiempo)):
  subList = tiempo[k:k+30]
  promedioMochilaMenorA3.append(promedio(subList))
  nMasmMenos1.append(m[k] + n[k])
  k += 30
print(len(promedioTiempoNporM))


promedio1NP = np.array(promedioTiempoNporM)
promedio2NP = np.array(promedioTiempoSumPocionesNecesarias)
promedio4NP = np.array(promedioMochilaMenorA3)
nm = np.array(nMasm)
nmUno = np.array(nMasmMenos1)
...
pylab.plot(nm, promedio2NP,color='green', marker='<', label= '100')
pylab.plot(nmUno, promedio4NP,color='blue', marker='v', label= 'entre 0 y 3')
